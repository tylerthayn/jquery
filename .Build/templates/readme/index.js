...
module.exports = (ast) => {
	...
	function Process (doclet, ast, indent = 0) {
		if (doclet.kind == 'class' || doclet.kind == 'module' || doclet.kind == 'namespace') {
			...
			Filter(ast, {memberof: doclet.longname, scope: 'static', kind: 'member'}).forEach(child => {
				Process(child, ast, indent+1)
				Filter(ast, {memberof: child.longname, scope: 'static', kind: 'member'}).forEach(child2 => Process(child2, ast, indent+2))
				Filter(ast, {memberof: child.longname, scope: 'static', kind: 'function'}).forEach(child2 => Process(child2, ast, indent+2))
			})
			Filter(ast, {memberof: doclet.longname, scope: 'static', kind: 'function'}).forEach(child => {
				Process(child, ast, indent+1)
				Filter(ast, {memberof: child.longname, scope: 'static', kind: 'member'}).forEach(child2 => Process(child2, ast, indent+2))
				Filter(ast, {memberof: child.longname, scope: 'static', kind: 'function'}).forEach(child2 => Process(child2, ast, indent+2))
			})
			log('')
			Filter(ast, {memberof: doclet.longname, scope: 'instance', kind: 'member'}).forEach(child => {
				Process(child, ast, indent+1)
				Filter(ast, {memberof: child.longname, scope: 'instance', kind: 'member'}).forEach(child2 => Process(child2, ast, indent+2))
				Filter(ast, {memberof: child.longname, scope: 'instance', kind: 'function'}).forEach(child2 => Process(child2, ast, indent+2))
			})
			Filter(ast, {memberof: doclet.longname, scope: 'instance', kind: 'function'}).forEach(child => {
				Process(child, ast, indent+1)
				Filter(ast, {memberof: child.longname, scope: 'instance', kind: 'member'}).forEach(child2 => Process(child2, ast, indent+2))
				Filter(ast, {memberof: child.longname, scope: 'instance', kind: 'function'}).forEach(child2 => Process(child2, ast, indent+2))
			})
			...
			log('\t'.repeat(indent)+doclet.name+'('+(doclet.params ? doclet.params.map(p => StringifyParam(p)).join(', ') : '')+')  '+doclet.Get('returns', []).map(ret => '{'+ret.Get('type.names', []).join('|')+'}'))
